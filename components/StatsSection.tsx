    ...
    <section className="border-2 border-border bg-surface p-6 sm:p-8 space-y-8">
      ...
      <div className="border-2 border-border p-4 sm:p-6 space-y-3">
        {CHAR_INFO.map(({ labelKey, valueKey }) => (
          <div key={labelKey} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3">
            <span className="text-accent-gold text-[8px] sm:text-[10px] min-w-[120px] sm:min-w-[140px]">
              {t(labelKey)}
            </span>
            <span className="text-text-primary text-[8px] sm:text-[10px]">
              {t(valueKey)}
            </span>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        {STAT_BOXES.map(({ labelKey, valueKey }) => (
          ...
          >
            <span className="text-accent-gold text-[7px] sm:text-[8px] tracking-wide mb-2">
              {t(labelKey)}
            </span>
      ...
      {/* Bio */}
      <div>
        <h3 className="text-accent-gold text-[9px] sm:text-[10px] tracking-wide mb-3">
          ─ {t("bio")} ─
        </h3>
        <p className="text-text-muted text-[7px] sm:text-[8px] leading-relaxed sm:leading-loose">
          {t("bioText")}
        </p>
      </div>
