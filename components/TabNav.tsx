    ...
    <nav className="flex border-b-2 border-border bg-surface">
      {TABS.map((tab) => (
        <button
          ...
          className={`relative px-2 sm:px-4 py-3 text-2xs sm:text-xs tracking-wide border-r-2 border-border transition-colors ${
            activeTab === tab
      ...
      <button
        onClick={onAskPablo}
        className="ml-auto px-2 sm:px-4 py-3 text-2xs sm:text-xs tracking-wide border-l-2 border-border text-accent-green hover:text-accent-gold hover:bg-background/50 transition-colors"
      >
        ✦ {t("askPablo")}
      </button>
    </nav>
